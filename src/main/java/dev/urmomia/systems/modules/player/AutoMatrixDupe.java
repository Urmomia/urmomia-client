...
import dev.urmomia.systems.modules.world.MountBypass;
...
public class AutoMatrixDupe extends Module {
    ...
    private final Setting<Boolean> shulkersOnly = sgGeneral.add(new BoolSetting.Builder()
            .name("shulker-only")
            .description("Only drop shulker boxes.")
            .defaultValue(true)
            .build()
    );
    ...
    public AutoMatrixDupe() {
        super(Categories.Player, "auto-matrix-dupe", "Does the Matrix Dupe for you, only works with llamas.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        //find carpet and select carpet
        if (slots == -1) {
            if (entity.hasChest() || mc.player.getMainHandStack().getItem() == Items.WHITE_CARPET){
                mc.player.networkHandler.sendPacket(new PlayerInteractEntityC2SPacket(entity, Hand.MAIN_HAND, mc.player.isSneaking()));
            } else {
                int slot = InvUtils.findItemWithCount(Items.WHITE_CARPET).slot;
                if (slot != -1 && slot < 9) {
                    mc.player.inventory.selectedSlot  = slot;
                 } else {
                    ChatUtils.moduleError(this, "Cannot find white carpets in your hotbar, disabling...");
                    this.toggle();
                }
            }
        } else if (slots == 0) {
            if (isDupeTime()) {
                ...
                int slot = InvUtils.findItemWithCount(Items.WHITE_CARPET).slot;
                InvUtils.dropSingle().slotId(slot);
            } else {
                mc.player.closeHandledScreen();
            ...
            if (mc.currentScreen instanceof HorseScreen && !(mc.player.currentScreenHandler.getSlot(0).getStack().getItem() == Items.WHITE_CARPET)) {
                int slot = InvUtils.findItemWithCount(Items.WHITE_CARPET).slot;
                InvUtils.quickMove().fromId(slot).toId(0);
            }
        } else if (slots > 0 ) {
            if (slotsToMove.isEmpty()) {
                ...
                if (empty) {
                    for (int i = slots + 2; i < mc.player.currentScreenHandler.getStacks().size(); i++) {
                        if (!(mc.player.currentScreenHandler.getStacks().get(i).isEmpty())) {
                            if (mc.player.currentScreenHandler.getSlot(i).getStack().getItem() == Items.WHITE_CARPET) continue;
            ...
            if (!slotsToMove.isEmpty()) {
                ...
                for (int i : slotsToMove) InvUtils.drop().slot(i);
                slotsToMove.clear();
            }
        }
    }
